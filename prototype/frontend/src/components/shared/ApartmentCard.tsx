      ...
      <CardContent className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-violet-600">{apartment.nearUniversity}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {apartment.facilities.slice(0, 4).map((facility) => (
            <span key={facility} className="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-700">
              {facility}
            </span>
          ))}
        </div>
      ...
      <CardContent className="grid gap-4 p-3 sm:grid-cols-[150px_minmax(0,1fr)_auto] sm:items-center">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <Link to={`/apartments/${apartment.id}`} className="break-words font-bold text-slate-950 hover:text-violet-700">
              {apartment.name}
            </Link>
            <StatusBadge status={apartment.availabilityStatus} />
          </div>
          <p className="mt-1 flex items-center gap-1 text-xs font-medium text-violet-600">
        ...
        <div className="flex items-center justify-between gap-4 sm:block sm:text-right">
          ...
          <div className="min-w-0 sm:clear-both sm:pt-7">
            ...
            <p className="text-xs text-slate-500">THB/month</p>
          </div>
        </div>
      </CardContent>
    </Card>
