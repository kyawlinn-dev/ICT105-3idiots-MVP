...
export function PublicNavbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur">
        ...
        <nav className="hidden items-center gap-3 md:flex">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                cn("rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-blue-50 hover:text-blue-700", isActive && "bg-blue-50 text-blue-700")
              }
            >
              {item.label}
            </NavLink>
          ))}
          <a href="/#about" className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-blue-50 hover:text-blue-700">
            About
          </a>
        </nav>
      ...
      {open ? (
        <div className="fixed inset-0 top-[57px] z-50 bg-slate-950/30 md:hidden" onClick={closeMenu}>
          <div className="ml-auto h-full w-full max-w-xs border-l border-slate-200 bg-white p-4 shadow-xl" onClick={(event) => event.stopPropagation()}>
            <nav className="grid gap-1">
              ...
              <a href="/#about" onClick={closeMenu} className="rounded-lg px-4 py-3 text-base font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-700">
                About
              </a>
